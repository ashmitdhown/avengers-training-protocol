...
export const analyzeImageCalories = async (imageFile) => {
  try {
    ...
    const payload = {
      contents: [{
        parts: [
          { text: 'Analyze this food. Respond strictly with a JSON object. Do not include markdown formatting blocks. The object must contain exactly these keys: "name" (string, capitalized), "calories" (number), "protein" (number), "carbs" (number), "fat" (number), "serving" (string), "confidence" (number 0-100), "comment" (short technical string), "tag" (one of: Meal, Snack, Drink, Protein, Vegetable, Fruit, Carbohydrate, Dairy, Fast Food, Healthy, Recovery).' },
          {
            inline_data: {
    ...
    if (!response.ok) {
      console.warn('Gemini API Limit Reached (429). Falling back to basic scanner protocol.');
      // Direct return fallback so Next.js dev overlay doesn't block the UI
      return {
        success: true,
        result: {
          name: 'Caloric Estimate (Fallback)',
          calories: 450,
          protein: 25,
          carbs: 45,
          fat: 15,
          serving: '1 plate',
          confidence: 25,
          comment: 'API Rate limit exceeded. Using generalized meal structure.',
          tag: 'Meal'
        }
      };
    }
    ...
    // Silent fallback to standard matching if API fails/limits
    return {
      success: true,
      result: {
        ...
        serving: '1 portion',
        confidence: 10,
        comment: 'Scanner offline or overloaded. Try manual entry.',
        tag: 'Meal'
      }
    };
  }
